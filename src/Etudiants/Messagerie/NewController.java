...
public class NewController  implements Initializable
{
       ...
       {   
           ...
           if(email.isBlank()||obj.isBlank()||contenu.isBlank())
           {
               ...
               for(String item : emails)
               {
                    ...
                    if(rs.next())
                    {   
                        PreparedStatement pstmt2=conn.prepareStatement("SELECT * FROM etudiant Where email_etudiant=?");
                        pstmt2.setString(1,item);
                        ResultSet rs2=pstmt2.executeQuery();
                        if(rs2.next())
                        {
                            Stage stage =new Stage();
                                Parent root=FXMLLoader.load(getClass().getResource("Recepteur.fxml")); 
                                Scene scene=new Scene(root);
                                stage.setResizable(false);
                                stage.initStyle(StageStyle.UNDECORATED);
                                stage.setScene(scene);
    ...
    public void initialize(URL url1, ResourceBundle rb) {
        try {
            ...
            while(res.next())
            {
                email_ =res.getString("email_etudiant");
                System.out.print(email_);
